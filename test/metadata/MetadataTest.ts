...
import {ModuleWithClassMapping} from "./data/ModuleWithClassMapping";
...
export class MetadataTest {
    ...
    @test("@Module Class mapping in metadata is parsed right")
    classMappingsAreParsedRight() {
        const context = new Context();
        context.configure(ModuleWithClassMapping);
        context.initialize();

        expect(
            context.injector.get(CustomModel),
            "Class mappings must be created anew each time"
        ).not.to.be.equals(context.injector.get(CustomModel));
    }
